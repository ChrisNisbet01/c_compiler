...
typedef struct source_location_entry_t {
    size_t preprocessed_line;      /* Line number in preprocessed file (key for lookup) */
...
void source_location_tracker_push_include(
    source_location_tracker_t * tracker,
    const char * filename,
    size_t line
);
...
const source_location_entry_t * source_location_tracker_find(
