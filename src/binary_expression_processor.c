...
binary_operation_mapping_t const shift_operations[] = {
    ...
    {LLVMBuildAShr, NULL, NULL, NULL, 0, 0, "ashr_tmp"}
};
...
size_t const compound_assignment_operations_count = sizeof(compound_assignment_operations) / sizeof(binary_operation_mapping_t);

// Helper function to promote integer types
static void promote_integer_operands(
...
static void promote_signed_integer_operands(
    ir_generator_ctx_t * ctx,
    TypedValue * lhs_res,
    TypedValue * rhs_res
) {
...
// Helper function for shift amount promotion/truncation
static void promote_shift_operands(
    ir_generator_ctx_t * ctx,
    TypedValue * lhs_res,
    TypedValue * rhs_res
) {
...
TypedValue process_binary_expression(
    ...
    if (strcmp(operation_name, "bitwise") == 0 || strcmp(operation_name, "shift") == 0) {
        promote_integer_operands(ctx, &lhs_res, &rhs_res);
    } else if (strcmp(operation_name, "arithmetic") == 0) {
        promote_signed_integer_operands(ctx, &lhs_res, &rhs_res);
    } else if (strcmp(operation_name, "shift") == 0) {
        promote_shift_operands(ctx, &lhs_res, &rhs_res);
    }
    // For relational and equality operations with integer operands, cast RHS to LHS type
    if ((strcmp(operation_name, "relational") == 0 || strcmp(operation_name, "equality") == 0) &&
        is_integer_kind(lhs_res.type_info) && is_integer_kind(rhs_res.type_info)) {
    ...
    size_t op_index = 0;
    if (strcmp(operation_name, "bitwise") == 0) {
        ...
        op_index = (size_t)op;
    } else if (strcmp(operation_name, "relational") == 0) {
        relational_operator_type_t op = op_node->op.rel.op;
        op_index = (size_t)op;
    } else if (strcmp(operation_name, "equality") == 0) {
    ...
    if (op_index >= op_map_size) {
        debug_error("Invalid operator index %zu for %s operation", op_index, operation_name);
        return NullTypedValue;
    }
    ...
    if (is_float_op && mapping->float_arith_op != NULL) {
        ...
        result_value = mapping->float_cmp_op(ctx->builder, mapping->float_predicate, lhs_res.value, rhs_res.value, mapping->name);
    } else if (!is_float_op && mapping->int_cmp_op != NULL) {
        result_value = mapping->int_cmp_op(ctx->builder, mapping->int_predicate, lhs_res.value, rhs_res.value, mapping->name);
    } else {
        debug_error("Unsupported operation for %s with float=%d", operation_name, is_float_op);
        return NullTypedValue;
    }

    // For relational and equality operations, we need to return a boolean type
    if (strcmp(operation_name, "relational") == 0 || strcmp(operation_name, "equality") == 0) {
        TypeDescriptor const * bool_type = get_or_create_builtin_type(
            ctx->type_descriptors, 
            (TypeSpecifier){.is_bool = true}, 
            (TypeQualifier){0}
        );
