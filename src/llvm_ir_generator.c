...
static TypedValue process_expression(ir_generator_ctx_t * ctx, c_grammar_node_t const * node);
...
register_enum_constants(ir_generator_ctx_t * ctx, c_grammar_node_t const * enum_node)
{
    ...
    for (size_t i = 0; i < enumerator_list->list.count; ++i)
    {
        ...
        if (child->type == AST_NODE_ENUMERATOR)
        {
            ...
            TypedValue val = (TypedValue){.value = global, .type = LLVMInt32TypeInContext(ctx->context)};
...
ir_generator_init(char const * module_name)
{
    ...
    // Add built-in macro __FILE__ as a string constant in the global scope
    {
        ...
        LLVMTypeRef arr_type = LLVMArrayType(LLVMInt8TypeInContext(ctx->context), (unsigned)(len + 1));
...
process_assignment(ir_generator_ctx_t * ctx, c_grammar_node_t const * node)
{
        ...
        if (op_node != NULL && op_node->op.unary.op == UNARY_OP_DEREF)
        {
            ...
            if (operand != NULL && operand->type == AST_NODE_IDENTIFIER)
            {
                ...
                if (find_symbol(ctx, ptr_name, &val))
                {
                    ...
                    LLVMValueRef addr = aligned_load(ctx->builder, val.type, val.value, "ptr_load");
    ...
    if (is_compound)
    {
        ...
        LLVMValueRef lhs_value = aligned_load(ctx->builder, lhs_res.type, lhs_res.value, "lhs_load");
    ...
    if (is_bitfield_assign && bitfield_struct_ptr != NULL && bitfield_struct_type != NULL)
    {
        ...
        // Store back
        aligned_store(ctx->builder, new_struct, bitfield_struct_type, bitfield_struct_ptr);
    }
    else
    {
...
process_identifier(ir_generator_ctx_t * ctx, c_grammar_node_t const * node)
{
    ...
    var_res = get_variable_pointer(ctx, node);

    if (var_res.value && var_res.type)
    {
        ...
        if (LLVMGetTypeKind(var_res.type) == LLVMArrayTypeKind)
        {
            ...
            return LLVMBuildInBoundsGEP2(ctx->builder, var_res.type, var_res.value, indices, 2, "array_ptr");
        }
        // Load the value from the memory address using LLVMBuildLoad2.
        return aligned_load(ctx->builder, var_res.type, var_res.value, "load_tmp");
    }
...
process_relational_expression(ir_generator_ctx_t * ctx, c_grammar_node_t const * node)
{
    ...
    res = cast_value_to_type(ctx, res, lhs_res.type, false);
...
process_equality_expression(ir_generator_ctx_t * ctx, c_grammar_node_t const * node)
{
    ...
    {
    case EQ_OP_EQ:
        res.value = is_float_op ? LLVMBuildFCmp(ctx->builder, LLVMRealOEQ, lhs_val, rhs_val, "feq_tmp")
                                : LLVMBuildICmp(ctx->builder, LLVMIntEQ, lhs_val, rhs_val, "eq_tmp");
        break;
    case EQ_OP_NE:
        res.value = is_float_op ? LLVMBuildFCmp(ctx->builder, LLVMRealONE, lhs_val, rhs_val, "fne_tmp")
...
process_unary_expression_prefix(ir_generator_ctx_t * ctx, c_grammar_node_t const * node)
{
    ...
    case UNARY_OP_DEC:
    {
        ...
        {
            LLVMValueRef original_val = aligned_load(ctx->builder, var_res.type, var_res.value, "orig_val");
                ...
                if (deref_operand && deref_operand->type == AST_NODE_IDENTIFIER)
                {
                    ...
                    if (find_symbol(ctx, var_name, &var))
                    {
                            ...
                            && LLVMGetTypeKind(var.type) == LLVMPointerTypeKind)
                        {
                            ...
                            var.pointee_type = LLVMInt32TypeInContext(ctx->context);
                        }
